from .syntax import Program, Term, Reference, Let, Immediate, Abstract, Apply, Primitive, Branch, Allocate, Load, Store, Begin
...
def _fold_term(term: Term) -> Term:
    match term:
        ...
        case Apply(target=target, arguments=args):
            return Apply(
                target=_fold_term(target),
                arguments=[_fold_term(arg) for arg in args]
            )
        ...
        case Store(base=base, index=idx, value=value):
            return Store(
                base=_fold_term(base),
                index=idx,
                value=_fold_term(value)
            )
        
        case Begin(effects=effects, value=value):
            return Begin(
                effects=[_fold_term(eff) for eff in effects],
                value=_fold_term(value)
            )
